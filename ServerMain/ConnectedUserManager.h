...
public:
	...
	void Init(const int maxSessionCount, TcpNet* pNetwork, ServerNetLib::ServerConfig* pConfig, ILog* pLogger)
	{
	...
	void LoginCheck()
	{
		...
		for (; m_LatestLogincheckIndex < lastCheckIndex; ++m_LatestLogincheckIndex)
		{
			...
			if (diff >= 180)
			{
				m_pRefLogger->Write(ServerNetLib::LOG_TYPE::L_WARN, "%s | Login Wait Time Over. sessionIndex(%d).", __FUNCTION__, i);
